...
Java_com_bata_localllm_llm_LlmBridge_loadModel(
        JNIEnv* env, jclass,
...
// Callback interface: void onToken(String token, boolean done)
JNIEXPORT void JNICALL
Java_com_bata_localllm_llm_LlmBridge_generate(
        JNIEnv* env, jclass,
...
JNIEXPORT void JNICALL
Java_com_bata_localllm_llm_LlmBridge_stopGeneration(JNIEnv*, jclass) {
    g_stop.store(true);
}

JNIEXPORT void JNICALL
Java_com_bata_localllm_llm_LlmBridge_freeModel(JNIEnv*, jclass, jlong handle) {
    if (!handle) return;
    auto* state = reinterpret_cast<ModelState*>(handle);
